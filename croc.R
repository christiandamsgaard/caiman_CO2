...
plot<-function (i,xtext) {
  ...
  model0<-lmer(paste(as.name(para[i]),"~(1|animalID)"),df);model0
  ...
  write.table(anova(model,model0)$`Pr(>Chisq)`[2],file = paste("./Data/Summarized data/",para[i],"_p-value for treatment.txt",sep = ""))
...
ggsave(filename = "./Figures/Figure 2.pdf",width = 9/2.54,height = 9/2.54)
ggsave(filename = "./Figures/Figure 2.jpeg",width = 9/2.54,height = 9/2.54,dpi = 600)
